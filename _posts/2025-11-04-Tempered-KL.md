...
Let $\Phi_\# g$ be the pushforward metric. Then  
$$  
...
- I use the heat kernel to define a tempered KL divergence, which is the KL divergence between the distributions after each is convolved with the heat kernel:  $$D_t(p||q) = D_{KL}(K_t*p||K_t*q)$$

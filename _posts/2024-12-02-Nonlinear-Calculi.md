...
$$
D_G f = e^{D[\ln f]} = e^{\left( \frac{Df}{f} \right)}
$$
...
$$
d_{\oplus}(a, b) = \int_a^b d\mu_{\oplus}(x)
$$
...
$f_+(x) = x^{12} $--->$f_{\oplus}(x) = x \otimes (e^{12})$
...
$$
D_{\oplus} f(x) = \lim_{\bar{x} \to x} \left[ f(\bar{x}) \ominus f(x)\right] \oslash d_{\oplus}(x, \bar{x})
$$
...
$$
D_{\oplus} [f \oplus g](x) = \lim_{\bar{x} \to x} \left[ (f(\bar{x}) \oplus g(\bar{x})) \ominus (f(x) \oplus g(x)) \right] \oslash d_{\oplus}(x, \bar{x})
$$
...
$$
D_{\oplus} [f \otimes g] = \left( D_{\oplus} [f] \otimes g \right) \oplus \left( f \otimes D_{\oplus} [g] \right)
$$

**Proof:**

$$
D_{\oplus} [f \otimes g](x) = \lim_{\bar{x} \to x} \left[  (f(\bar{x}) \otimes g(\bar{x})) \ominus (f(x) \otimes g(x)) \right] \oslash \left[ d_{\oplus}(x, \bar{x}) \right]
$$
...
(I haven't investigated this any further, but it seems to me like using another distance d(x,xbar) should get you another version of the derivative which should also work just fine as a derivative - I'm just taking the Haar one as canonical because that agrees with standard geometric calculus)

### Definite Integral

$$
J^{[a, b]}_{\oplus} [f] = \lim_{N \to \infty} \bigoplus_{n=0}^N \left[ f(x_n) \otimes \mu(x_n, x_{n-1}) \right]
$$
...
$$
TS_{\oplus}(x) = \bigoplus_{n=0}^{\infty} \left[ a_n \otimes P_{\oplus, n}(x) \right]
$$
...
$$
a \oplus b = \frac{1}{\frac{1}{a} + \frac{1}{b}}
$$
...
$$
Dg = f^{-1}\left[ f'(g(x)) \cdot g'(x) \right]
$$
...
$$
P_n(x) = f^{-1}(\frac{x^n}{n!})
$$

(Geometric calculus has twisting function $f_G(x) = ln(x)$ and harmonic calculus has $f_H(x) = 1/x$.  If you want a laugh, you can also do $f(x)=e^{\alpha x}$, which in the limit $\alpha$ to infinity gives you the 'tropical' calculus)
...
As an example with functions, if you have some $p_+$, you get $p_{\times}$ by rewriting all the operators in $p_+$ by their $\oplus$ counterparts.  E.g. $p_+(x) = x^3 = x * x * x$, so $p_{\times}(x) = x \otimes x \otimes x = x^{ln(x)^{ln(x)}}$.  $p_+$" is the representation of $p$ in the standard field, and $p_{\times}$ is the representation of $p$ in the geometric world (the x^... bit is just a tutorial on how to calculate that).
...
$$
D_{\oplus} y_{\oplus} \oplus \left( q_{\oplus}(x) \otimes q_{\oplus}(x) \right) \otimes y = p_{\oplus}(x)
$$

or less irritatingly:

$$
D y \oplus \left( \left( q(x) \otimes q(x) \right) \otimes y \right) = p(x)
$$
...
$$
y = A x + B + J^{[0, x]}_{x_1} J^{[0, x_1]}_{x_2} p_H(x_2)
$$

And now we transform back by reinterpreting:

$$
\frac{1}{y} = \frac{1}{Ax} + \frac{1}{B} + \frac{1}{ \int_0^x{\frac{dx_1} { \left[ \int_0^{x_1} \frac{dx_2}{[p_+(x_2)]} \right]}}}
$$
...
$$
f * E_1 = E_2 = \left[ \text{sym if sym.is\_variable()} \text{ else } \text{convert\_to\_O}(sym) \right]
$$
...
$$
[f * E_1](x) = E_2(x) = f^{-1}\left(E_1(f(x))\right)
$$

And also:

$$
E_1(x) = f\left([f * E_1](f^{-1}(x))\right)
$$
...
Call this expression $G_1(z, \{b_n\})$.$g(z)$ is equal at all orders to the series, without arranging terms or anything:

$$
g(z) = f^{-1}(b_0) \oplus f^{-1}(z) \otimes f^{-1}(b_1) \oplus \dots = \bigoplus_{n=0}^{\infty} \left[ f^{-1}(z)^n \otimes f^{-1} (b_n) \right]
$$

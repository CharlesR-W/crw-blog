...
Can we use this to say when the formal sequence $G_1$ will converge? Plug in the identity above $E_1(x) = f([f * E_1](f^{-1}(x)))$ (“Real Taylor series = $f$ (pseudo-Taylor series with different args)”—they are equal at each step in the sequence).

We know the following: $[f^{-1} * G_2](z, \{ b_n \})$ converges for $|f^{-1}(z)| < R$, and that it is equal to the right-hand side of the above in value at every order of partial approximant. Therefore, the sequence of partial approximants (still for a different series than we started with!)
...
$$
b_0 + z \cdot f(b_1 + z \cdot f(b_2 + \dots)) = \bigoplus_{n=0}^{\infty} [f^{-1}(z)^n \otimes f^{-1}(b_n)] = f^{-1}\left( \sum_{n=0}^{\infty} b_n f\left( [f^{-1}(z)]^n \right) \right).
$$

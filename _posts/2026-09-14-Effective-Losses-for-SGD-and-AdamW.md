...
$$\boxed{\;\dot\theta = -\nabla\Big(L + \frac{\eta}{4}\|\nabla L\|^2\Big) - \eta^2\Big(\tfrac13 H^2 g + \tfrac1{12} T[g,g]\Big) + O(\eta^3).\;}$$

The $O(\eta)$ term is **implicit gradient regularization (IGR)**: the discrete iterates follow, to this order, gradient flow on $\tilde L = L + \frac{\eta}{4}\|\nabla L\|^2$.
...
- **IGR is a per-mode rescaling of the learning rate.** $\nabla \frac{\eta}{4}\|g\|^2 = \frac{\eta}{2} H g$ exactly, so the modified flow is $\dot\theta = -(1 + \frac{\eta}{2}H)\nabla L$: a preconditioned flow whose preconditioner boosts sharp modes by the factor $(1 + \eta\lambda_k/2)$. There is no $\nabla\lambda$ (flatness-seeking) force in IGR at this order; the "prefers flat minima" story of Barrett and Dherin is about which trajectory you end up on, via $\|\nabla L\|^2 \approx d^\top H^2 d$ being larger near sharp minima at equal $L$.
...
$$\mathcal L^{(1)} = \frac{\eta}{4}\|\nabla L\|^2, \qquad \text{per-mode strength } \frac{\eta\lambda_k}{2}.$$

For SGD, the per-step version of this is the BEA of the *minibatch* map, giving $\frac{\eta}{4}\|\nabla \hat L_t\|^2$ step by step; averaging over an epoch is what produces the next term.
...
$$\Big\langle \sum_k \hat H_k \sum_{j<k}\hat g_j\Big\rangle = \frac12\Big(m^2 Hg - \sum_k \hat H_k \hat g_k\Big) = \frac{m^2}{2}Hg - \frac14 \nabla\sum_k \|\hat g_k\|^2,$$

since $\hat H_k \hat g_k = \frac12 \nabla\|\hat g_k\|^2$. Therefore $\nabla R = \frac{1}{4m}\nabla\sum_k\|\hat g_k\|^2$ and

$$\boxed{\;\tilde L_{\rm SGD} = L + \frac{\eta}{4m}\sum_{k=1}^m \|\nabla \hat L_k\|^2 = L + \frac{\eta}{4}\|\nabla L\|^2 + \frac{\eta}{4m}\sum_{k=1}^m\|\nabla\hat L_k - \nabla L\|^2,\;}$$

the cross term vanishing because $\sum_k \xi_k = 0$. This is the result of [Smith, Dherin, Barrett and De (2021)](https://arxiv.org/abs/2101.12176). For $B \ll N$, $\frac1m\sum_k\|\xi_k\|^2 \approx \frac{1}{B}\,{\rm tr}\,\Sigma$ (finite-population factor $\frac{N-B}{N-1}$ if you want it), so
...
$$\boxed{\;C_{kk} = \frac{\eta\,\Sigma_{kk}}{B\,\lambda_k\,(2 - \eta\lambda_k)}\;\xrightarrow{\;\eta\lambda_k\ll 1\;}\;\frac{\eta\,\Sigma_{kk}}{2B\lambda_k},\;}\qquad \tau_k = \frac{-1}{\ln|1-\eta\lambda_k|}\approx \frac{1}{\eta\lambda_k}.$$

This is the discrete fluctuation–dissipation relation. It is the same object as the stationary covariance of the stochastic modified equation of [Li, Tai and E (2017)](https://arxiv.org/abs/1511.06251), $d\theta = -\nabla(L + \frac\eta4\|\nabla L\|^2)\,dt + \sqrt{\eta}\,\Sigma_B^{1/2}\,dW$, whose drift — note — contains only the IGR term, consistent with 4a for i.i.d. sampling.

Plug into the flatness force:

$$\frac{d\bar\theta}{dt}\Big|_{\rm noise} = -\frac{\eta}{2}\sum_k C_{kk}\nabla\lambda_k = -\frac{\eta^2}{2B}\sum_k \frac{\Sigma_{kk}}{\lambda_k(2-\eta\lambda_k)}\nabla\lambda_k .$$
...
Fixed point: $(2\eta\gamma - \eta^2\gamma^2) r_*^2 = \eta^2\hat G/r_*^2$, i.e.

$$r_*^4 = \frac{\eta\,\hat G}{\gamma(2-\eta\gamma)}\approx\frac{\eta\hat G}{2\gamma}.$$

Linearizing $x = r^2$ about $x_*$: $\partial_x[-2\eta\gamma x + \eta^2\hat G/x] = -2\eta\gamma - \eta^2\hat G/x_*^2 = -4\eta\gamma$, so the norm relaxes in $\tau_{\rm norm} = 1/(4\eta\gamma)$ steps (for an Adam-like update whose norm does not depend on $r$, the second contribution is absent and $\tau_{\rm norm} = 1/(2\eta\gamma)$).

**Angular dynamics (fast).** The direction moves by $\tan\Delta\phi = \frac{\eta\|g\|}{(1-\eta\gamma)r}\approx \frac{\eta\sqrt{\hat G}}{r^2}$, which at equilibrium is

$$\boxed{\;\Delta\phi_* = \sqrt{2\eta\gamma}\;\text{ per step, independent of }\hat G,\qquad \eta_{\rm eff} := \frac{\eta}{r_*^2} = \sqrt{\frac{2\eta\gamma}{\hat G}}.\;}$$

The effective learning rate on the sphere is set by the *intrinsic* rate $\eta\gamma$ ([Li, Lyu and Arora 2020](https://arxiv.org/abs/2010.02916)), the per-step rotation is universal ([Wan et al. 2020](https://arxiv.org/abs/2006.08419), "spherical motion dynamics"; [Kosson, Messmer and Jaggi 2024](https://arxiv.org/abs/2305.17212), "rotational equilibrium", who extend it to AdamW and Lion and use it to explain why AdamW beats Adam with $L^2$). The angular motion is just GD on the sphere with a slowly varying learning rate $\eta_{\rm eff}(t) = \eta/r_t^2$; everything in Sections 3–5 applies on the sphere with $\eta \to \eta_{\rm eff}$ and $H \to$ the spherical Hessian $r^2 H|_{\perp w}$.
...
$$\boxed{\;\tilde L_{\rm HB} = L + \frac{\eta}{4}\cdot\frac{1+\beta}{1-\beta}\,\|\nabla L\|^2,\;}$$
...
**AdamW.** Decoupled decay $\theta \leftarrow (1-\eta\gamma)\theta - \eta\,m/\sqrt\nu$ on a scale-invariant block gives the Section 6 equilibrium with an update norm $\|u\|$ that does not depend on $r$: $r_* = \sqrt{\eta/(2\gamma)}\,\|u\|$, rotation $\sqrt{2\eta\gamma}$ per step, $\tau_{\rm norm} = 1/(2\eta\gamma)$. For white-noise gradients each coordinate of $u = m/\sqrt\nu$ has variance $\frac{1-\beta_1}{1+\beta_1}$, for coherent gradients it is $\pm1$, so $\|u\|^2 \in [\frac{1-\beta_1}{1+\beta_1}d,\ d]$ for a block of $d$ parameters. This is Kosson et al.'s rotational equilibrium; the universality of $\sqrt{2\eta\gamma}$ across optimizers is why AdamW's $(\eta,\gamma)$ pair, rather than either alone, is the thing to tune.

**Summary of the answer.** Adam *is* an effective-loss story in the window: after the momentum transient, in the overdamped band, on windows short compared to $1/(1-\beta_2)$, and away from EoS — a $P$-gradient flow of $L + \frac{\eta}{4}\frac{1+\beta_1}{1-\beta_1}\|\nabla L\|^2_{P^{-1}}$ plus the whitened-noise flatness force. It stops being one (a) in the underdamped band, where modes ring; (b) when $\nu$'s lag matters, which flips the sign of the IGR-like term; (c) at EoS, where you need the phase-space rod and $\nu$'s response. All three are timescale statements, which is why the $\beta_1 \ll \beta_2$ (in $1-\beta$) convention is not just a heuristic.
...
$$\frac{d\bar\theta}{dt} = -\eta\,\nabla_{\rm semi}\,\mathcal L_{\rm eff},\qquad \mathcal L_{\rm eff} = L + \underbrace{\frac{\eta}{4}\|\nabla L\|^2}_{\text{IGR}} + \underbrace{\frac{\eta}{4B}{\rm tr}\,\Sigma}_{\text{reshuffle}} + \underbrace{\tfrac12\langle H, C_{\rm noise}\rangle}_{\text{thermal flatness}} + \underbrace{\tfrac12\langle H, C_{\rm osc}\rangle}_{\text{EoS flatness}} + \frac{\gamma}{2}\|\theta\|^2 ,$$
...
and $\nabla_{\rm semi}$ meaning: differentiate $L$, $\|\nabla L\|^2$, ${\rm tr}\,\Sigma$ and $\|\theta\|^2$ fully, but differentiate $\langle H, C\rangle$ only through $H$. Which terms are honest gradients: the first three and the last. The two flatness terms are semi-gradients; the EoS one is a constraint force (Lagrangian $L + \mu(\lambda_1 - 2/\eta)$ with $\mu = \frac{\sigma^2}{2}$), and the thermal one is a gradient only in special cases (${\rm tr}\,H$ for aligned noise, $\log\det H$ for isotropic noise). Momentum: multiply the IGR term by $\frac{1+\beta}{1-\beta}$ and use the underdamped relaxation time in the closures. Adam: replace $\|\cdot\|^2$ by $\|\cdot\|^2_{P^{-1}}$, $\Sigma$ by $P^{-1}\Sigma P^{-1}$, and read the whole thing in the $P$-metric.
...
Per mode $k$ define $g_k$, $\lambda_k$, $\Sigma_{kk}$, and the **curvature length** $\ell_k = \lambda_k/\|\nabla\lambda_k\|$ (distance over which the mode's sharpness changes by order one).
...
| $F_k = \dfrac{C_{kk}\|\nabla\lambda_k\|}{2\|\nabla L\|}$ | flatness force / loss force | thermal: $\dfrac{\eta\Sigma_{kk}}{2B\,\ell_k\|\nabla L\|(2-\epsilon_k)}$; EoS (central flow): $|\cos\angle(\nabla\lambda_k,\nabla L)|$. |
...
| ${\rm SNR}^{\rm coord}_j = Bg_j^2/\Sigma_{jj}$ | Adam's preconditioner regime | $\gg1$: sign-descent-like, $\|\cdot\|_1$ IGR; $\ll1$: whitened noise, temperature $\eta^2$. |
...
Timescales, in steps: $2$ (EoS oscillation); $1/(1-\beta_1)$; $1/(\eta\lambda_k)$ per mode; $1/(1-\beta_2)$; $m = N/B$; $1/(4\eta\gamma)$ and $1/(2\eta\gamma)$; the sharpness-drift time $\lambda_k/(\eta\, C_{kk}\|\nabla\lambda_k\|^2) = \ell_k^2/(\eta\lambda_k A_k^2)$; the spike-emergence time of Section 10 (data-dependent; I don't have a formula).

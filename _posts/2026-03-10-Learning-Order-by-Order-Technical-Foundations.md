...
*Work in progress from [MATS](https://www.matsprogram.org/) 9.0 under the mentorship of Richard Ngo.  Technical companion to [Learning, Order by Order]({{ site.baseurl }}/Learning-Order-by-Order/) - each section here is self-contained and develops one piece of the mathematical machinery.  Prerequisite: the main note's setup (learner tensors $\mathcal{K}_n$, graded dual pairing, two hat matrices).*
...
At order $n$, the coefficient in $\Delta(y)$ is $\frac{1}{(n-1)!} - \frac{1}{n!} = \frac{n-1}{n!}$, which vanishes at $n = 1$.  So $\Delta(y) = 0$ if and only if the learner is linear.  The Frobenius norm $\|\Delta(y)\|_F$ is a pointwise measure of nonlinearity, and its expected value $\mathbb{E}[\|\Delta(y)\|_F]$ measures the average nonlinearity of the learner under the data distribution.
...
This is the **Born series**.  The $k$-th term has $k$ insertions of $\Sigma$ between $k+1$ bare resolvents.  It converges absolutely when $\|\Sigma(y)\, G_z^{(0)}\|_{\text{op}} < 1$ (operator norm) - when the nonlinear corrections are small relative to the regularized linear response.
...
The capacity in each mode **decays exponentially with depth**, with rate $\gamma_k$ (the Lyapunov exponent for mode $k$).

### An analogy: Anderson localization of capacity (speculative)

The exponential decay of capacity through products of random transfer matrices is structurally parallel to **Anderson localization** in disordered systems, where products of random transfer matrices produce exponential decay of quantum-mechanical wavefunctions.  The mathematical structure is genuinely shared: products of random $2 \times 2$ matrices, Lyapunov exponents, Furstenberg's theorem.

(This analogy was suggested by Claude during our research conversations and I have not yet fully evaluated how deep it runs.  The underlying transfer matrix mathematics is correct, but the physical intuitions should be taken as suggestive rather than established.)
...
where $|0\rangle = (1, 0, 0, \ldots)$ is the vacuum (the "no data" state).  This is the exponential generating function for the symmetric tensor powers.

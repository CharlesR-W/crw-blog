...
The common thread in all of these is that 'exactly equal to', 'is a minimum', and 'is modular' are all claims which cannot survive contact with finite-precision measurement.  The fix is always the same: replace the binary question with a quantitative one, and state the scale at which you're asking it.

There is a deep humility in this, which I think is worth sitting with.  All of our knowledge is bounded - every claim we make about the world is hedged by the scale at which we made it.  But this is not cause for despair; it's the opposite.  It is precisely the admission that we cannot see everything which permits us to say anything at all.  An unbounded claim is not a stronger one - it is a vacuous one.  The constraint is what gives our statements their content, in the same way that the axioms of a mathematical structure are not limitations but the very thing which makes theorems possible.

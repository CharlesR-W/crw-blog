...
$$
\boxed{P(\lambda_1, \ldots, \lambda_N) \propto \prod_{i < j} \vert \lambda_i - \lambda_j\vert ^2 \cdot \prod_i e^{-\frac{N}{2}\lambda_i^2}}
$$

That $\vert \lambda_i - \lambda_j\vert ^2$ factor is the **Vandermonde determinant** squared. It's a repulsive interaction between eigenvalues—they don't want to be near each other. This is eigenvalue repulsion, and it's what makes RMT nontrivial.

Rewrite this in Boltzmann form:

$$
P(\boldsymbol{\lambda}) \propto \exp\left( -N \sum_i \frac{\lambda_i^2}{2} + 2\sum_{i < j} \log\vert \lambda_i - \lambda_j\vert  \right)
$$
...
$$
-\frac{1}{N^2} \log P \approx \int d\lambda \, \frac{\lambda^2}{2} \rho(\lambda) - \int d\lambda \, d\mu \, \rho(\lambda) \rho(\mu) \log\vert \lambda - \mu\vert 
$$
...
$$
\frac{\lambda^2}{2} = \int d\mu \, \rho(\mu) \log\vert \lambda - \mu\vert  + \text{const}
$$
...
$$
\boxed{\rho(\lambda) = \frac{1}{2\pi} \sqrt{4 - \lambda^2}, \quad \vert \lambda\vert  \leq 2}
$$
...
$$
P(\boldsymbol{\lambda}) \propto \prod_{i<j} \vert \lambda_i - \lambda_j\vert ^\beta \cdot \prod_i e^{-\frac{\beta N}{4} \lambda_i^2}
$$
...
- Left tail: $f(s) \sim e^{-\vert s\vert ^3/12}$ (superexponential decay)

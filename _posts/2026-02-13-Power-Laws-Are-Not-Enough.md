...
If scaling laws reflect the covariance and the covariance alone, then understanding what produces intelligence requires understanding what natural data has beyond its covariance.  It obviously involves higher-order statistical structure, but that's nearly tautological.  You can try and get another few more orders of moments by polyspectra, and there are some interesting results, but I don't think it will give satisfying answers - long-range correlations are too hard to resolve this way.
